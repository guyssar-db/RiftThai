<script lang="ts">
	...
	const qaData: {
		...
		qa_list: [
			...spiritforgedFaq,
			{
				...
				question:
					'สกิล Soaring Scout ที่เขียนว่า DEATHKNELL: Channel 1 rune exhausted ทำงานอย่างไร',
				answer:
			...
			},
			{
				...
				question:
					'การ์ดเวทมนคร์ Consult the Past ที่มีคีย์เวิร์ด Hidden ต้องจ่ายค่าร่ายอย่างไรและวางไว้ตรงไหน',
				answer:
			...
			},
			{
				category: 'Card Mechanics',
				question:
					'เมื่อเปิดใช้เวทย์ Spirit Call เสก Spirit Token ออกมา โทเคนนั้นจะลงสนามรบที่ไหนและมีสภาพอย่างไร',
				answer:
			...
			},
			{
				...
				question:
					'ยูนิตสามารถสั่ง Move (เคลื่อนที่) กระโดดข้ามเลนจาก Battlefield หนึ่งไปอีก Battlefield หนึ่งได้ไหม?',
				answer:
			...
			},
			{
				...
				question:
					'สามารถใช้ Spirit Token ที่เพิ่งเสกออกมาเปิดทำ Showdown (ศึกตัดสินพื้นที่) ในเทิร์นนั้นเลยได้ไหม?',
				answer:
			...
			},
			{
				category: 'Card Mechanics',
				question:
					'การ์ด Cull the Weak ที่สั่งให้ผู้เล่นแต่ละคนทำลายยูนิตตัวเอง 1 ตัว หากบอร์ดเราว่างเปล่าจะร่ายได้ไหม?',
				answer:
			...
			},
			{
				category: 'Card Mechanics',
				question:
					'ถ้าเราเล่นเด็ค Viktor, Innovator แล้วสั่งเปิดหงายการ์ดเวทย์ที่หมอบ Hidden ไว้ในเทิร์นคู่แข่ง สกิลเสกทหารจะทำงานไหม?',
				answer:
			...
			},
			{
				...
				question:
					'คะแนนโบนัสจากสถานะ Hold และ สกิลของ Ahri, Alluring จะทำงานและแจกแต้มให้ในจังหวะไหน?',
				answer:
			...
			},
			{
				category: 'Card Mechanics',
				question:
					'เมื่อ Glasc Mixologist ตายลง ยูนิตตัวใหม่ที่ชุบชีวิตขึ้นมาจะได้เข้าสู้ต่อในไฟต์เดิมไหม?',
				answer:
			...
			},
			{
				...
				question:
					'ในการจ่ายค่าร่าย Power Rune โดยวิธี Recycle เราจำเป็นต้องเลือกรูนที่ตั้งตรง (Ready) เท่านั้นใช่ไหม?',
				answer:
			...
			},
			{
				category: 'Rules & Movement',
				question:
					'Spell ที่เขียนว่า Move a unit ย้าย unit จาก battlefield หนึ่งไปอีก battlefield ได้ไหม?',
				answer:
	...
	let filteredQA = $derived(
		qaData.qa_list.filter((item: QAItem) => {
...
<div class="rt-page-shell font-sans text-slate-100">
	...
	<div class="rt-container py-4 sm:py-8">
		...
		<div class="mx-auto mb-8 max-w-3xl space-y-3 text-center">
			<p class="rt-kicker">ถาม–ตอบกติกา</p>
			...
			<div class="relative mx-auto mt-7 max-w-3xl">
				<input
					...
					class="w-full rounded-lg border border-white/10 bg-slate-950/70 p-4 text-white transition-all placeholder:text-slate-600 focus:border-cyan-300/50 focus:ring-4 focus:ring-cyan-400/10 focus:outline-none"
				/>
			</div>

			<div class="rt-panel mx-auto mt-3 max-w-3xl rounded-xl p-2">
				<div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
					<select
						bind:value={selectedCategory}
						class="w-full rounded-lg border border-white/10 bg-slate-950 px-4 py-3 text-xs font-black tracking-widest text-white uppercase focus:border-cyan-300 focus:outline-none"
					>
					...
					{#if searchTerm || selectedCategory !== 'All'}
						<button
							type="button"
							class="rounded-lg border border-white/10 px-4 py-3 text-xs font-black tracking-widest text-slate-400 uppercase transition active:bg-slate-800"
				...
				<div class="hidden">
					{#each categories as category}
						<button
							type="button"
							class="min-h-11 rounded-lg border px-3 py-2 text-[10px] font-black tracking-widest uppercase transition focus:outline-none focus-visible:ring-4 focus-visible:ring-cyan-400/25 {selectedCategory ===
							category
								? 'border-cyan-300 bg-cyan-300 text-slate-950'
								: 'border-white/10 bg-slate-950 text-slate-400 hover:border-slate-700 hover:text-cyan-300'}"
					...
					{#if searchTerm || selectedCategory !== 'All'}
						<button
							type="button"
							class="min-h-11 rounded-lg border border-white/10 px-3 py-2 text-[10px] font-black tracking-widest text-slate-500 uppercase transition hover:text-white focus:outline-none focus-visible:ring-4 focus-visible:ring-cyan-400/25"
							onclick={resetFilters}
						>
							Reset
						</button>
			...
			<div class="space-y-1 text-xs font-black tracking-widest text-slate-600 uppercase">
				<div>{filteredQA.length} / {qaData.qa_list.length} Questions</div>
				<div class="font-semibold tracking-normal text-slate-500 normal-case">
		...
		<div class="mx-auto max-w-5xl space-y-3">
			{#each filteredQA as item}
				<details
					class="rt-panel group overflow-hidden rounded-xl transition-all open:border-cyan-300/25"
				>
					<summary
						class="flex min-h-16 cursor-pointer list-none items-start justify-between gap-4 p-5 transition hover:bg-white/[0.025] focus:outline-none focus-visible:ring-4 focus-visible:ring-cyan-400/25 focus-visible:ring-inset sm:p-6 [&::-webkit-details-marker]:hidden"
					>
						<div class="min-w-0 space-y-3">
							<span
								class="inline-flex rounded-lg border border-white/10 bg-slate-950 px-3 py-1 text-[0.6rem] font-black tracking-widest text-cyan-300 uppercase"
							>
								{item.category}
							</span>
							<h2 class="text-base leading-relaxed font-bold text-white sm:text-lg">
								{item.question}
							</h2>
						</div>
						<svg
							class="mt-1 h-5 w-5 shrink-0 text-cyan-300 transition-transform duration-200 group-open:rotate-180"
							viewBox="0 0 20 20"
							fill="currentColor"
							aria-hidden="true"
						>
					...
					<div class="border-t border-white/10 px-5 pt-4 pb-5 sm:px-6 sm:pb-6">
						<p
							class="rounded-lg border border-white/10 bg-slate-950/60 p-4 leading-relaxed font-medium text-slate-300"
						>
							{item.answer}
						</p>
						{#if item.source}
							<a
								class="mt-3 inline-flex min-h-11 items-center rounded-lg px-2 text-xs font-black tracking-widest text-cyan-300 uppercase transition hover:bg-cyan-400/10 hover:text-cyan-100 focus:outline-none focus-visible:ring-4 focus-visible:ring-cyan-400/25"
								href={item.source}
								target="_blank"
								rel="noreferrer"
							>
