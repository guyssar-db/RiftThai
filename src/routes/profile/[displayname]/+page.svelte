...
<div class="rt-page-shell min-h-dvh pb-16 text-slate-100">
	...
	<main class="rt-container py-6 sm:py-10">
		<header class="rt-panel rt-topline rt-scanline relative mb-6 overflow-hidden rounded-xl">
			<div
				class="pointer-events-none absolute -top-24 -right-20 h-72 w-72 rounded-full bg-cyan-300/12 blur-3xl"
			></div>
			<div class="rt-rule-line relative p-5 pl-7 sm:p-7 sm:pl-9 lg:p-8 lg:pl-10">
				<div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:gap-8">
					<div class="min-w-0 flex-1">
						<p class="rt-kicker mb-2">โปรไฟล์ผู้เล่น</p>
						...
						<div class="mt-4 flex flex-wrap items-center gap-3 text-xs sm:text-sm">
							<span
								class="rounded-lg border border-white/10 bg-slate-950/40 px-3 py-1.5 font-mono text-slate-300"
							>
								@{profile.profileHandle}
							</span>
							<span
								class="flex items-center gap-2 rounded-lg border border-cyan-300/10 bg-cyan-300/5 px-3 py-1.5 text-cyan-200"
							>
								<svg
							...
							<span
								class="flex items-center gap-2 rounded-lg border border-orange-400/10 bg-orange-400/5 px-3 py-1.5 text-orange-200"
							>
								<svg
		...
		{:else}
			<section class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
				{#each decks as deck}
					...
					<a
						href="/deck/{deck.id}"
						class="rt-panel group grid grid-cols-[7.5rem_minmax(0,1fr)] rounded-xl p-3 transition hover:border-cyan-300/30 hover:bg-cyan-300/5 focus:border-cyan-300/60 focus:ring-1 focus:ring-cyan-300/20 focus:outline-none"
					>
						<div class="overflow-hidden rounded-lg border border-white/10 bg-black/20">
							...
							{:else}
								<div
									class="flex aspect-[744/1039] w-full items-center justify-center bg-slate-900/60 text-slate-600"
								>
									<svg
										class="h-8 w-8"
										viewBox="0 0 24 24"
						...
						<div class="flex min-w-0 flex-col justify-between p-2 pl-3">
							<div class="min-w-0">
								<h2
									class="truncate text-lg font-black text-white uppercase italic transition group-hover:text-cyan-300"
								>
									{deck.name}
								</h2>
								<p class="mt-1 text-[9px] font-black tracking-widest text-slate-500 uppercase">
									{summary.stats.total} ใบ
								</p>
								<div class="mt-3 flex flex-wrap gap-1.5">
									{#each summary.domains as domain}
										<span
											class="inline-flex items-center gap-1 rounded-md border border-white/5 bg-black/40 px-2 py-0.5 text-[10px] font-black text-cyan-100"
										>
							...
							<div class="mt-4 flex items-center justify-between">
								...
								<span
									class="flex items-center gap-1 text-[10px] font-black tracking-widest text-cyan-300 uppercase transition group-hover:text-cyan-100"
								>
									ดูเด็ค
									<svg
										class="h-3 w-3"
