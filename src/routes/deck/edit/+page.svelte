<script lang="ts">
	...
	$effect(() => {
		...
		deckNameInput = getActiveStoredDeck(nextCollection).name;
		isDeckLoading = false;
	});
	...
	function clearDeck() {
		const nextCollection = updateActiveDeckChampion(updateActiveDeckEntries(collection, []), '');
		saveCollection(nextCollection);
		entries = getActiveStoredDeck(nextCollection).entries;
		isClearConfirmOpen = false;
	}
	...
	function clampQuantity(card: Card, quantity: number) {
		...
		if (isMainDeckCard(card)) {
			const usedBySameName = getQuantityByName(card) - current;
	...
	<main class="rt-container py-6 sm:py-10">
		...
		<section class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
			<div class="min-w-0">
				...
				<div class="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-4 xl:grid-cols-4 2xl:grid-cols-5">
					{#each paginatedCards as card}
						{@const quantity = getCardQuantity(entries, card.code)}
							...
							<div class="relative flex aspect-[744/1039] w-full items-center justify-center overflow-hidden bg-slate-950">
								{#if card.image_url}
									<img
										...
										class="h-full w-full object-contain transition duration-500 {isBattlefieldCard(card) ? '-rotate-90 scale-135 group-hover:scale-140' : 'group-hover:scale-105'}"
										alt={card.name_en}
										loading="lazy"
									/>
								{:else}
								...
								{#if getRarityIcon(card.rarity)}
									<div class="absolute left-2 top-2 grid h-8 w-8 place-items-center rounded-full border border-white/10 bg-slate-950/85 shadow-lg backdrop-blur">
			...
			<aside class="rt-panel fixed inset-y-0 left-0 z-[925] w-[min(24rem,calc(100vw-1.5rem))] overflow-hidden rounded-r-xl border-r border-cyan-300/20 p-5 shadow-2xl shadow-black/50 transition-transform duration-300 {isDeckDrawerOpen ? 'translate-x-0' : '-translate-x-full'} lg:sticky lg:top-[4.75rem] lg:z-auto lg:h-fit lg:w-auto lg:translate-x-0 lg:rounded-xl lg:border-r-0">
				...
				<div class="mb-4 grid grid-cols-3 gap-2 text-center text-[9px] font-black uppercase tracking-widest">
					<div class="rounded-md border border-white/10 bg-black/20 p-2">
						<div class="text-sm text-white">{stats.mainTotal}/{maxMainDeckCards}</div>
						<div class="mt-1 text-slate-500">Main</div>
					</div>
					<div class="rounded-md border border-white/10 bg-black/20 p-2">
						<div class="text-sm text-white">{stats.runeTotal}/{maxRuneCards}</div>
						<div class="mt-1 text-slate-500">Rune</div>
					</div>
					<div class="rounded-md border border-white/10 bg-black/20 p-2">
						<div class="text-sm text-white">{stats.battlefieldTotal}</div>
				...
				<div class="h-[calc(100dvh-22rem)] space-y-2 overflow-y-auto pr-1 lg:h-auto lg:max-h-[70dvh]">
					{#each deckCards.filter((item) => !isLegendCard(item.card)) as item}
						<div class="relative overflow-hidden rounded-lg border border-white/10 bg-black/20 {isBattlefieldCard(item.card) ? 'sm:col-span-1' : ''}">
							{#if item.card.image_url}
								<img
									src={getCardImageUrl(item.card.image_url, 260, 'webp')}
									class="absolute inset-0 h-full w-full object-cover opacity-100 {isBattlefieldCard(item.card) ? 'object-center translate-x-6' : 'object-[55%_15%] translate-x-6'}"
									alt=""
									loading="lazy"
								/>
							{/if}
							...
							<div class="relative flex {isBattlefieldCard(item.card) ? 'min-h-15' : 'min-h-15'} items-center justify-between gap-3 p-3">
