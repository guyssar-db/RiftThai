<script lang="ts">
	...
	let selectedDeck = $derived(loadedDeck ?? collection?.decks.find((deck) => deck.id === deckId) ?? null);
	...
	let collectionStats = $derived.by(() => {
		if (!userCardCollection || Object.keys(userCardCollection).length === 0 || !selectedDeck) return null;
		...
		const entriesList = [
			...(selectedDeck.entries ?? []),
			...(selectedDeck.sideboardEntries ?? [])
		];
	...
	function copyMissingList(missingList: { card: Card; quantity: number }[]) {
		const text = missingList
			.map(item => `${item.quantity}x ${item.card.name_en}`)
			.join('\n');
		...
		{#if selectedDeck}
							...
							{#if isLocal && (selectedDeck.source !== 'online' || !selectedDeck.onlineId)}
								<div class="mt-4 rounded-lg border border-amber-300/20 bg-amber-400/5 p-3.5 text-xs text-amber-200">
									<div class="flex items-start gap-2.5">
										...
										<div>
											<span class="font-black uppercase tracking-wider block mb-1">Local Deck (บันทึกเฉพาะในเครื่องนี้)</span>
											คนอื่นจะไม่สามารถเข้าดูลิงก์นี้ได้ หากต้องการแชร์ให้เพื่อน กรุณาไปที่ <a href="/deck" class="font-bold underline text-white hover:text-cyan-300">คลังเด็ค</a> แล้วกดปุ่มสามจุด (&vellip;) เพื่อ <strong>บันทึกออนไลน์ (Save Online)</strong> และตั้งค่าเป็น <strong>สาธารณะ (Publish)</strong>
										</div>
									</div>
								</div>
							{:else if selectedDeck.onlineId}
								<div class="mt-4 flex flex-wrap gap-2">
									...
									{:else if selectedDeck.visibility === 'unlisted'}
										<span class="inline-flex items-center gap-1.5 rounded-full border border-cyan-500/20 bg-cyan-500/10 px-2.5 py-1 text-[10px] font-black text-cyan-300 uppercase tracking-widest">
											<span class="h-1.5 w-1.5 rounded-full bg-cyan-400"></span>
											Unlisted (ไม่เป็นสาธารณะ - ดูได้เฉพาะผู้มีลิงก์)
										</span>
									{:else}
										<span class="inline-flex items-center gap-1.5 rounded-full border border-slate-500/20 bg-slate-500/10 px-2.5 py-1 text-[10px] font-black text-slate-400 uppercase tracking-widest">
						...
						<div class="flex flex-wrap gap-2">
							...
							{#if hasDeck}
								<button
									...
									class="inline-flex min-h-11 items-center rounded-lg border border-cyan-300/20 bg-cyan-300/8 px-4 text-xs font-black tracking-widest text-cyan-100 uppercase transition hover:bg-cyan-300/14 hover:text-white cursor-pointer"
								>
									Playtest
								</button>
							{/if}
				...
				<div class="mb-6 rounded-xl border border-amber-300/20 bg-amber-400/5 p-4 sm:p-5">
					<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
						<div class="flex items-start gap-3">
							<div class="rounded-full bg-amber-400 p-2 text-slate-950 mt-0.5 shadow-lg">
							...
							<div>
								...
								<p class="mt-1 text-xs text-slate-300 leading-relaxed">
									คุณสะสมการ์ดในเด็คนี้ได้ {collectionStats.ownedTotal} / {collectionStats.requiredTotal} ใบ (ขาดอีก {collectionStats.missingList.length} แบบ)
								</p>
							</div>
						</div>
						...
						<button
							type="button"
							class="inline-flex min-h-10 items-center justify-center rounded-lg border border-amber-200/20 bg-amber-400/10 px-4 text-xs font-black tracking-widest text-amber-200 uppercase transition hover:bg-amber-400/20 cursor-pointer self-start sm:self-center"
			...
			{:else if collectionStats && collectionStats.missingCount === 0}
				<div class="mb-6 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 sm:p-5">
					<div class="flex items-start gap-3">
						<div class="rounded-full bg-emerald-500 p-2 text-slate-950 mt-0.5 shadow-lg">
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
						...
						<div>
							...
							<p class="mt-1 text-xs text-slate-300 leading-relaxed">
								คุณมีการ์ดสะสมทั้งหมด {collectionStats.requiredTotal} / {collectionStats.requiredTotal} ใบที่เด็คนี้ต้องการในบัญชีของคุณเรียบร้อยแล้ว
							</p>
						</div>
					</div>
				</div>
			...
			{#if !hasDeck}
				<section class="rt-panel rounded-xl p-8 text-center">
					...
					{:else}
						<button onclick={cloneDeckToLocal} class="rt-action cursor-pointer mt-6">Copy to My Decks</button>
					{/if}
				</section>
			{:else}
