<script lang="ts">
	...
	let openDeckMenuDeck = $derived(collection.decks.find((deck) => deck.id === openDeckMenuId) ?? null);
	let deleteDeckTarget = $derived(collection.decks.find((deck) => deck.id === deleteDeckId) ?? null);
	let activeFilterCount = $derived(
		[zoneFilter, domainFilter, setFilter].filter((value) => value !== 'all' && value !== 'All').length
	);
	...
	function changeQuantity(card: Card, delta: number) {
		...
		if (isEditingSideboard) {
			...
			setSideboard(setCardQuantity(sideboardEntries, card.code, clampSideboardQuantity(card, current + delta)));
			return;
		}
	...
	function setQuantity(card: Card, quantity: number) {
		...
		if (isEditingSideboard) {
			setSideboard(setCardQuantity(sideboardEntries, card.code, clampSideboardQuantity(card, quantity)));
			return;
		}
	...
	function selectLegend(card: Card) {
		const nextEntries = [
			...entries.filter((entry) => {
				...
				return entryCard ? !isLegendCard(entryCard) && isCardAllowedForLegend(entryCard, card) : true;
			}),
	...
	function toggleDeckMenu(deckId: string, target: HTMLElement) {
		...
		deckMenuLeft = Math.max(12, Math.min(rect.right - menuWidth, window.innerWidth - menuWidth - 12));
	...
	function setChampion(card: Card) {
		...
		const nextCollection = updateActiveDeckChampion(updateActiveDeckEntries(collection, entries), card.code);
		saveCollection(nextCollection);
		entries = getActiveStoredDeck(nextCollection).entries;
	}
	...
	function canIncrease(card: Card) {
		...
		if (isEditingSideboard) {
			return getCardQuantity(sideboardEntries, card.code) < clampSideboardQuantity(card, getCardQuantity(sideboardEntries, card.code) + 1);
		}
		return getCardQuantity(entries, card.code) < clampQuantity(card, getCardQuantity(entries, card.code) + 1);
	}
	...
	function getLimitLabel(card: Card) {
		...
		if (isChampionCandidate(card, selectedLegend)) return `main ${stats.mainTotal}/${maxMainDeckCards}`;
	...
	function getVisiblePages() {
		...
		if (totalCardPages <= visibleCount) return Array.from({ length: totalCardPages }, (_, index) => index + 1);
		if (cardPage <= 3) return Array.from({ length: visibleCount }, (_, index) => index + 1);
		if (cardPage >= totalCardPages - 2) {
			return Array.from({ length: visibleCount }, (_, index) => totalCardPages - visibleCount + 1 + index);
		}
	...
	<main class="rt-container py-6 sm:py-10">
					...
					<div class="flex flex-col space-y-2">
						<div class="flex flex-wrap items-end gap-2">
							<label class="grid min-w-[220px] flex-1 gap-1 sm:flex-none">
								<span class="text-[10px] font-black uppercase tracking-widest text-slate-500">Deck Name</span>
								<input
							...
							<button
								type="button"
								class="inline-flex min-h-11 items-center rounded-lg border border-rose-300/20 px-4 text-xs font-black uppercase tracking-widest text-rose-100 transition hover:bg-rose-500/10 disabled:opacity-40"
		...
		<section class="rt-panel relative mb-4 rounded-xl p-3">
			<div class="flex gap-2 overflow-x-auto pb-1">
				<button
					...
					onclick={createDeck}
				>
					<span class="text-xs font-black uppercase tracking-widest text-cyan-100">+ New Deck</span>
				</button>
				{#each collection.decks as deck}
					<div class="relative min-w-48">
					<button
						type="button"
						class="min-h-16 w-full rounded-lg border p-3 text-left transition {deck.id === collection.activeDeckId ? 'border-cyan-300/50 bg-cyan-300/10 shadow-[0_0_24px_rgba(83,234,253,0.08)]' : 'border-white/10 bg-black/15 hover:border-cyan-300/20 hover:bg-white/5'}"
						onclick={() => selectStoredDeck(deck.id)}
					>
						<span class="block truncate pr-9 text-sm font-black uppercase italic text-white">{deck.name}</span>
						<span class="mt-1 block text-[10px] font-black uppercase tracking-widest text-slate-500">
							{deck.entries.reduce((total, entry) => total + entry.quantity, 0)} Cards
						</span>
					</button>
					<button
						type="button"
						class="absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-md border border-white/10 bg-slate-950/80 text-lg font-black text-slate-300 transition hover:bg-white/10 hover:text-white"
						onclick={(event) => {
				...
				{#each legendCards as card}
					...
					>
						<img
							...
							class="aspect-[744/1039] w-20 shrink-0 rounded-md object-cover pointer-events-none"
							alt={card.name_en}
							loading="lazy"
						/>
						<div class="min-w-0">
							<div class="truncate text-sm font-black text-white">{card.name_en}</div>
							<div class="mt-1 text-[10px] font-black uppercase tracking-widest text-slate-500">{card.code}</div>
		...
		<section class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
			<div class="min-w-0">
					...
					<div class="flex flex-col gap-3 xl:flex-row">
						<div class="flex min-w-0 flex-1 gap-2">
							<input
								...
								class="min-h-12 min-w-0 flex-1 rounded-md border border-white/10 bg-[#080b12]/80 px-4 text-sm font-medium text-white placeholder:text-slate-600 focus:border-amber-200/50 focus:outline-none focus:ring-4 focus:ring-amber-200/10"
								placeholder="Search card name, code, type, domain..."
							/>
							<button
								type="button"
								class="relative grid min-h-12 w-12 shrink-0 place-items-center rounded-md border border-white/10 bg-[#080b12]/80 text-white transition active:scale-95 focus:border-amber-200/50 focus:outline-none focus:ring-4 focus:ring-amber-200/10 xl:hidden"
						...
						<div class="hidden gap-2 sm:grid-cols-3 xl:grid xl:min-w-[620px]">
							...
							<IconSelect bind:value={domainFilter} label="All Colors" options={domainFilterOptions} />
				...
				<div class="mb-4 flex flex-col gap-3 rounded-xl border border-white/10 bg-black/15 p-3 sm:flex-row sm:items-center sm:justify-between">
					<div class="text-xs font-black uppercase tracking-widest text-slate-500">
						Showing {(cardPage - 1) * cardsPerPage + (paginatedCards.length > 0 ? 1 : 0)}-{Math.min(cardPage * cardsPerPage, filteredCards.length)}
						of {filteredCards.length} cards
					</div>
					{@render CardPagination(false)}
				</div>

				<div class="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-4 xl:grid-cols-4 2xl:grid-cols-5">
					{#each paginatedCards as card}
						{@const quantity = isEditingSideboard ? getCardQuantity(sideboardEntries, card.code) : getCardQuantity(entries, card.code)}
						<article class="group overflow-hidden rounded-xl border border-white/10 bg-black/20 transition hover:-translate-y-1 hover:border-cyan-300/35 hover:shadow-[0_0_36px_rgba(45,212,191,0.10)]">
							<div class="relative flex aspect-[744/1039] w-full items-center justify-center overflow-hidden bg-slate-950">
								{#if card.image_url}
									<img
										src={getCardImageUrl(card.image_url, 320, 'webp')}
										class="h-full w-full object-contain transition duration-500 {card.type === 'Battlefield' ? '-rotate-90 scale-135 group-hover:scale-140' : 'group-hover:scale-105'}"
										alt={card.name_en}
										loading="lazy"
									/>
								{:else}
								...
								<div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/25 to-transparent opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100"></div>

								{#if getRarityIcon(card.rarity)}
									<div class="absolute left-2 top-2 grid h-8 w-8 place-items-center rounded-full border border-white/10 bg-slate-950/85 opacity-0 shadow-lg backdrop-blur transition group-hover:opacity-100 group-focus-within:opacity-100">
								...
								<button
									type="button"
									class="absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-full border border-white/15 bg-slate-950/85 text-sm font-black text-cyan-100 shadow-lg backdrop-blur transition hover:bg-cyan-300 hover:text-slate-950"
								...
								{#if quantity > 0}
									<div class="absolute right-2 top-12 rounded-md bg-amber-200 px-2 py-1 text-xs font-black text-slate-950 shadow-lg">
										x{quantity}
									</div>
								{/if}

								<div class="absolute inset-x-0 bottom-0 p-2 opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100">
									<div class="mb-2 min-w-0">
										<div class="truncate text-xs font-black uppercase italic text-white drop-shadow sm:text-sm">{card.name_en}</div>
										<div class="mt-1 flex min-w-0 items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-300">
											{#each getTypeIcons(card.type) as typeIcon}
												<img src="/images/icons/{typeIcon.src}" class="h-4 w-4 shrink-0 object-contain" alt={typeIcon.label} />
											{/each}
										...
										{#if getLimitLabel(card)}
											<div class="mt-1 hidden text-[9px] font-black uppercase tracking-widest text-amber-100 sm:block">{getLimitLabel(card)}</div>
										{/if}
									</div>
									...
									{#if isChampionCandidate(card, selectedLegend)}
										<button
											type="button"
											class="mb-2 min-h-8 w-full rounded-md border bg-slate-950/85 px-2 text-[9px] font-black uppercase tracking-widest backdrop-blur transition sm:min-h-9 sm:px-3 sm:text-[10px] {card.code === activeDeck.championCode ? 'border-cyan-300/40 bg-cyan-300/15 text-cyan-100' : 'border-amber-200/20 text-amber-100 hover:bg-amber-200/10'}"
									...
									<div class="grid grid-cols-[2rem_1fr_2rem] items-center gap-1 sm:grid-cols-[2.5rem_1fr_2.5rem]">
										<button
											type="button"
										...
										<button
											...
											onclick={() => isLegendCard(card) ? selectLegend(card) : changeQuantity(card, 1)}
											aria-label="Add card"
										>
											+
										</button>
			...
			<aside class="rt-panel fixed inset-y-0 left-0 z-[925] w-[min(24rem,calc(100vw-1.5rem))] overflow-hidden rounded-r-xl border-r border-cyan-300/20 p-5 shadow-2xl shadow-black/50 transition-transform duration-300 {isDeckDrawerOpen ? 'translate-x-0' : '-translate-x-full'} lg:sticky lg:top-[4.75rem] lg:z-auto lg:h-fit lg:w-auto lg:translate-x-0 lg:rounded-xl lg:border-r-0">
				<div class="mb-4 flex items-center justify-between gap-3">
					<h2 class="text-lg font-black uppercase italic text-white">Current Deck</h2>
					<button
						type="button"
				...
				<div class="mb-4 inline-flex w-full items-center rounded-lg border border-white/10 bg-slate-950/40 p-1">
					<button
						type="button"
						class="flex-1 rounded-md py-2 text-center text-xs font-black uppercase tracking-widest transition {!isEditingSideboard ? 'bg-cyan-300 text-slate-950' : 'text-slate-400 hover:text-white'}"
				...
				<div class="mb-4 grid grid-cols-4 gap-1.5 text-center text-[8px] sm:text-[9px] font-black uppercase tracking-wider">
					<div class="rounded-md border border-white/10 bg-black/20 p-2">
						<div class="text-[11px] sm:text-xs text-white">{stats.mainTotal}</div>
						<div class="mt-1 text-slate-500">Main</div>
					</div>
					<div class="rounded-md border border-white/10 bg-black/20 p-2">
						<div class="text-[11px] sm:text-xs text-white">{stats.runeTotal}</div>
						<div class="mt-1 text-slate-500">Rune</div>
					</div>
					<div class="rounded-md border border-white/10 bg-black/20 p-2">
						<div class="text-[11px] sm:text-xs text-white">{stats.battlefieldTotal}</div>
						<div class="mt-1 text-slate-500">Field</div>
					</div>
					<div class="rounded-md border border-white/10 bg-black/20 p-2">
						<div class="text-[11px] sm:text-xs text-white">{stats.sideboardTotal}/{maxSideboardCards}</div>
				...
				<div class="mb-4 grid grid-cols-2 gap-2">
					...
					{#if championCard}
						<div class="relative overflow-hidden rounded-lg border border-cyan-300/20 bg-black/20">
							...
							<button
								type="button"
								class="absolute right-2 top-2 z-10 grid h-8 w-8 place-items-center rounded-md border border-rose-300/25 bg-slate-950/85 text-sm font-black text-rose-100 shadow-lg backdrop-blur transition hover:bg-rose-500/20 hover:text-white"
					...
					{:else if selectedLegend}
						<div class="grid aspect-[744/1039] place-items-center rounded-lg border border-dashed border-white/10 bg-black/20 p-3 text-center text-[10px] font-black uppercase tracking-widest text-slate-500">
							No Champion
						</div>
					{/if}
				</div>
				<div class="h-[calc(100dvh-22rem)] space-y-2 overflow-y-auto pr-1 lg:h-auto lg:max-h-[70dvh]">
					{#each (isEditingSideboard ? sideboardCards : deckCards).filter((item) => !isLegendCard(item.card)) as item}
						<div class="relative overflow-hidden rounded-lg border border-white/10 bg-black/20 {item.card.type === 'Battlefield' ? 'sm:col-span-1' : ''}">
							{#if item.card.image_url}
								<img
									src={getCardImageUrl(item.card.image_url, 260, 'webp')}
									class="absolute inset-0 h-full w-full object-cover opacity-100 {item.card.type === 'Battlefield' ? 'object-center translate-x-6' : 'object-[55%_15%] translate-x-6'}"
									alt=""
									loading="lazy"
								/>
							{/if}
							<div class="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/80 to-slate-950/0"></div>
							<div class="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-slate-950/0 to-transparent"></div>
							<div class="relative flex {item.card.type === 'Battlefield' ? 'min-h-15' : 'min-h-15'} items-center justify-between gap-3 p-3">
								<div class="min-w-0 pr-4">
									<div class="truncate text-sm font-black text-white drop-shadow">{item.card.name_en}</div>
									<!-- <div class="text-[10px] font-black uppercase tracking-widest text-slate-300/80">{item.card.code} | {item.card.type}</div> -->
								</div>
								<div class="flex shrink-0 items-center gap-2">
									{#if isChampionCandidate(item.card, selectedLegend)}
										<button
											type="button"
											class="grid h-8 w-8 place-items-center rounded-md border text-sm font-black transition {item.card.code === activeDeck.championCode ? 'border-cyan-300/40 bg-cyan-300/15 text-cyan-100' : 'border-amber-200/25 bg-slate-950/80 text-amber-100 hover:bg-amber-200/10'}"
											disabled={item.card.code === activeDeck.championCode}
											onclick={() => setChampion(item.card)}
											aria-label={item.card.code === activeDeck.championCode ? 'Current champion' : 'Set as champion'}
											title={item.card.code === activeDeck.championCode ? 'Current champion' : 'Set as champion'}
										>
											♛
										</button>
									{/if}
...
{#if openDeckMenuDeck}
	...
	>
		<div class="truncate px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500">
			{openDeckMenuDeck.name}
		</div>
		<button
			type="button"
			class="block min-h-10 w-full rounded-md px-3 text-left text-xs font-black uppercase tracking-widest text-slate-200 transition hover:bg-white/10 hover:text-white"
		...
		<button
			type="button"
			class="block min-h-10 w-full rounded-md px-3 text-left text-xs font-black uppercase tracking-widest text-rose-100 transition hover:bg-rose-500/15"
...
{#if selectedLegend}
	...
	>
		...
		<span class="absolute -right-1 -top-1 grid h-6 min-w-6 place-items-center rounded-full bg-cyan-300 px-1 text-[10px] font-black text-slate-950">
			{stats.total}
		</span>
	</button>
{/if}
...
{#if deleteDeckTarget}
	<div class="fixed inset-0 z-[980] grid place-items-center bg-slate-950/80 p-4 backdrop-blur-sm">
		<div class="rt-panel w-full max-w-md rounded-xl border border-rose-300/20 p-5 shadow-2xl shadow-rose-950/30">
			<p class="rt-kicker mb-3 text-rose-100">Confirm Delete</p>
			<h2 class="text-2xl font-black uppercase italic text-white">Delete Deck?</h2>
			<p class="rt-copy mt-3 text-sm">
				This will permanently delete <span class="font-black text-white">{deleteDeckTarget.name}</span> from this browser.
			</p>
			<div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
				<button
					type="button"
					class="inline-flex min-h-11 items-center justify-center rounded-lg border border-white/10 px-4 text-xs font-black uppercase tracking-widest text-slate-300 transition hover:bg-white/5 hover:text-white"
				...
				<button
					type="button"
					class="inline-flex min-h-11 items-center justify-center rounded-lg border border-rose-300/25 bg-rose-500/15 px-4 text-xs font-black uppercase tracking-widest text-rose-50 transition hover:bg-rose-500/25"
...
{#if isClearConfirmOpen}
	<div class="fixed inset-0 z-[980] grid place-items-center bg-slate-950/80 p-4 backdrop-blur-sm">
		<div class="rt-panel w-full max-w-md rounded-xl border border-rose-300/20 p-5 shadow-2xl shadow-rose-950/30">
			...
			<h2 class="text-2xl font-black uppercase italic text-white">Clear Current Deck?</h2>
			<p class="rt-copy mt-3 text-sm">
				This will remove all cards, legend, and champion from <span class="font-black text-white">{activeDeck.name}</span>.
			</p>
			<div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
				<button
					type="button"
...
{#snippet CardPagination(spaced: boolean)}
	{#if totalCardPages > 1}
		<div class="{spaced ? 'mt-6 flex justify-center' : ''}">
			<div class="flex max-w-full items-center justify-center gap-1 overflow-x-auto rounded-lg border border-white/10 bg-slate-950/70 p-1">
				<button
					type="button"
				...
				{#each getVisiblePages() as page}
					<button
						type="button"
						class="grid h-9 w-9 shrink-0 place-items-center rounded-md text-xs font-black transition {cardPage === page ? 'bg-cyan-300 text-slate-950' : 'text-slate-400 hover:bg-white/5 hover:text-white'}"
