...
set
	profile_number = profile_candidates.profile_number,
	profile_slug = profile_candidates.base_slug || '-' || profile_candidates.profile_number
from profile_candidates
where public.app_users.id = profile_candidates.id
	and (
		public.app_users.profile_number is null
		or public.app_users.profile_slug is null
		or public.app_users.profile_slug !~ '-[0-9]{5}$'
	);
